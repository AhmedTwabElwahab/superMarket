...
$(document).ready(function()
{
    ...
    let purchase_price       = $('input[id=purchase_price]');
    ...
    function Search()
    {
        ...
        $("#purchaseReturn tr").filter(function()
        {
    ...
    product.on('change',function ()
    {
        $.ajax({
            ...
            success: function(data)
            {
                if (data !== null)
                {
                    console.log(data.stock[0]['purchases_quantity']);
                    if (data.stock[0]['purchases_quantity'] !== 0)
                    {
                        ...
                        Max_return.val(data.stock[0]['purchases_quantity']);
                        quantity.val(1);
                        purchase_price.val(data.product[0]['purchase_price']);
    ...
    $(document).on('click','.btn-add',function ()
    {
        ...
        if (barcode.val() === '' || product.val() === '' || purchase_price.val() === '' || quantity.val() === '')
        {
            return 0;
        }
        ...
        let n = $( "tr" ).length;
        $('#purchaseReturn').find('tbody').append(
            ...
            "<td><input class='input-hide' min='0' step='0.01' id='price"+ n +"' onchange='CalcTotalRow("+n+")' style=\"width: 100px;-moz-appearance: textfield;\" type=\"number\" value='"    + purchase_price.val() + "' name=\"purchase_price_new_[] \"></td>\n" +
            "<td><input  type='number' class='total_row' id='total"+ n +"' name='total_row_new[]' value='"+ purchase_price.val()*quantity.val() +"' style=\"width: 100px;-moz-appearance: textfield;\" readonly>"+"</td>"+
    ...
    $('#purchaseReturn').on('click','tbody tr td button#remove_row',function ()
    {
