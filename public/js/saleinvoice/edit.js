...
$(document).ready(function()
{
    ...
    product.on('change',function ()
    {
        $.ajax({
            ...
            success: function(data)
            {
                if (data !== null)
                {
                    console.log(data.product[0]['unit']['name']);
                    if (data.stock[0]['available'] !== 0)
                    {
    ...
    $(document).on('click','.btn-add',function ()
    {
        ...
        $('#saleInvoices').find('tbody').append(
            ...
            "<td id='available"+ n +"' data-value='" + available_quantity.val() +"'>" + available_quantity.val() +"</td>\n" +
            "<td>" + product_unit.val() +"</td>\n" +
            "<td><input min='0' id='quantity"+ n +"' onchange='CalcTotalRow("+n+")' style=\"width: 100px;-moz-appearance: textfield;\" type=\"number\" value='" + quantity.val() + "' name=\"quantity_new_[] \"></td>\n" +
            "<td><input min='0' id='price"+ n +"' onchange='CalcTotalRow("+n+")' style=\"width: 100px;-moz-appearance: textfield;\" type=\"number\" value='"    + sale_price.val() + "' name=\"sale_price_new_[] \"></td>\n" +
