...
@section('content')
    ...
    <div class="row container-body">
        ...
        <div class="col-md-12">
            <div class="row ">
                <form class="form" action="{{route('purchaseReturn.update',$purchaseReturn->id)}}"
                      method="POST"
                            ...
                            <select id="supplier_id_input" class="form-control"
                                ...
                                @if(!empty($suppliers))
                                    @foreach($suppliers as $supplier)
                                        <option value="{{$supplier->id}}" @if($supplier->id == $purchaseReturn->supplier->id)selected @endif>{{$supplier->name}}</option>
                                    @endforeach
                                @endif
                            </select>
                            ...
                            <select id="payment_type_id_input" class="form-control" name="payment_type_id" required>
                                @foreach($payments as $payment)
                                    @if($payment->id != PAYMENT_VISA)
                                        <option value="{{$payment->id}}" @if($purchaseReturn->payment_type_id === $payment->id) selected @endif>{{$payment->name}}</option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select id="warehouse_input" class="form-control" name="warehouse" disabled required>
                                @foreach($warehouses as $warehouse)
                                    <option value="{{$warehouse->id}}" @if($purchaseReturn->warehouse_id === $warehouse->id) selected @endif>{{$warehouse->name}}</option>
                                @endforeach
                            </select>
                            @error("warehouse")
                                <span class="text-danger">{{$message}}</span>
                            ...
                            <tbody>
                                ...
                                @foreach($purchaseReturn->details as $index => $detail)
                                    <tr>
                                        ...
                                        <td id='Max_return{{$index}}' data-value=" {{$detail->product->stockCustom($purchaseReturn->warehouse_id)->purchases_quantity}}">
                                            {{$detail->product->stockCustom($purchaseReturn->warehouse_id)->purchases_quantity}}
                                        </td>
                                        <td>
